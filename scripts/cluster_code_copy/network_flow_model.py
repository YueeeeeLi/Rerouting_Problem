...
# drop the disrupted roads if necessary (optional)
user_input = input("Please choose a scenario (base/flooded): ")
#if len(flooded_road_list) > 0:
...
# add toll costs to road links 
tolls = pd.read_csv(base_path / "networks"/ "road" / "tolls.csv")
road_link_file["average_toll_cost"] = 0
tolls_mapping = tolls.iloc[1:, :].set_index("e_id")["Average_cost (£/passage)"].to_dict()
road_link_file["average_toll_cost"] = road_link_file["e_id"].apply(lambda x: tolls_mapping.get(x, 0))
road_link_file.loc[road_link_file.road_classification_number == "M6", "average_toll_cost"] = 8.0
...
speed_dict, acc_flow_dict, acc_capacity_dict = func.network_flow_model(
    test_net_ig,  # network
    edge_cost_dict, #!!! operating costs
    road_link_file,  # road
...
# %%
# export file
output_file_name = str(user_input) + "_scenario_gb_edge_flow.gpkg"
road_link_file.to_file(
    base_path / ".." / "outputs" / output_file_name,
    driver="GPKG",
)
